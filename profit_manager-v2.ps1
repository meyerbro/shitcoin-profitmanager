...
$best_coin = Invoke-RestMethod -Uri "https://minecryptonight.net/api/best" -Method Get 

Write-Host "...Best Coin to Mine:" $best_coin.current


if ($best_coin.current -eq 'XTL')
{
...
$best_coin_check = Invoke-RestMethod -Uri "https://minecryptonight.net/api/best" -Method Get
$TimeStart = Get-Date
...
Do { 
 $TimeNow = Get-Date
  ...
  Write-Host "...Best Coin to Mine:" $best_coin_check.current
  if ($best_coin.current -eq $best_coin_check.current) {
  $set_sleep = "50"
  ...
  Write-Host $TimeNow : "Currently mining"$best_coin.current": Checking again at $TimeEnd"
 }
 Start-Sleep -Seconds 10
}
While ($best_coin.current -eq $best_coin_check.current)
Write-Host "Profitability has changed, switching now"
Write-Host "Shutting down miner, please wait..... "
Stop-Process -Name $miner_type
Start-Sleep -s 8
.\profit_manager-v2.ps1